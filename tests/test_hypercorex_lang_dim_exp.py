...
# Some parameters about the language recognition set
NUM_CLASSES = 21
NUM_FEATURES = 128
NUM_PREDICTIONS = 10
NUM_HV_EXPANSION = 16
...
async def tb_hypercorex_dut(dut):
    cocotb.log.info(" ------------------------------------------ ")
    cocotb.log.info("         Testing lang Recognition         ")
    cocotb.log.info(" ------------------------------------------ ")

    # Extract data set
    assoc_mem_fp = get_dir() + "/../hemaia/trained_am/hypx_lang_am_multi.txt"
    ...
    # Extract samples
    test_samples_fp = get_dir() + "/../hemaia/test_samples/hypx_lang_nsample2_test.txt"
    cocotb.log.info(f"Get test samples: {test_samples_fp}")
    ...
    print(len(test_samples_orig))
    print(len(test_samples))
    print(len(test_samples[0]))

    # Extract asm file
    inst_file_path = get_dir() + "/../sw/asm/test_lang_recog.asm"
    ...
    # Load list to A
    test_sub_samples = test_samples[: NUM_PREDICTIONS * NUM_HV_EXPANSION]
    test_sub_samples_len = len(test_samples[0])
    ...
    check_result(loop_count1, read_loop_count)
    ...
    [
        {
            ...
            "TbMemDepthLowDimA": str(
                NUM_FEATURES * NUM_PREDICTIONS * NUM_HV_EXPANSION * 5
            ),
            "TbAMMemDepth": str(NUM_CLASSES * NUM_HV_EXPANSION),
        }
    ],
)
def test_hypercorex_lang_dim_exp(simulator, parameters, waves):
    ...
    module = "test_hypercorex_lang_dim_exp"
