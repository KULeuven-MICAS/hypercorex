...
# Some parameters about the uciharacter recognition set
NUM_CLASSES = 6
NUM_FEATURES = 561
NUM_PREDICTIONS = 10
NUM_HV_EXPANSION = 8  # Number of times to expand the HV dimension
...
async def tb_hypercorex_dut(dut):
    cocotb.log.info(" ------------------------------------------ ")
    cocotb.log.info("         Testing ucihar Recognition         ")
    cocotb.log.info(" ------------------------------------------ ")

    # Extract data set
    assoc_mem_fp = get_dir() + "/../hemaia/trained_am/hypx_ucihar_am_multi.txt"
    ...
    # Extract samples
    test_samples_fp = get_dir() + "/../hemaia/test_samples/hypx_ucihar_nsample_test.txt"
    ...
    # Extract asm file
    inst_file_path = get_dir() + "/../sw/asm/train_ucihar_recog.asm"
    ...
    # Write loop count set
    loop_count1 = await config_inst_loop_count(
        dut=dut,
        reg_addr=set_parameters.INST_LOOP_COUNT1_REG_ADDR,
        val1=NUM_FEATURES,
        val2=NUM_HV_EXPANSION * NUM_PREDICTIONS,
    )
    ...
    read_loop_count1 = await read_csr(dut, set_parameters.INST_LOOP_COUNT1_REG_ADDR)
    check_result(loop_count1, read_loop_count1)
    ...
    [
        {
            ...
            "TbMemDepthLowDimA": str(NUM_FEATURES * NUM_HV_EXPANSION * 5),
            "TbAMMemDepth": str(NUM_CLASSES * NUM_HV_EXPANSION),
        }
    ],
)
def test_hypercorex_ucihar_dim_exp(simulator, parameters, waves):
    ...
    module = "test_hypercorex_ucihar_dim_exp"
