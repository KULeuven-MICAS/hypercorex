...
async def tb_hypercorex_dut(dut):
    ...
    for i in range(len(dataset)):
        dataset_compressed.append(hvlist2num(dataset[i]))

    # Extract asm file
    inst_file_path = get_dir() + "/../sw/asm/train_char_recog_data_slice.asm"
    ...
    # We start at 2 because the first 2 elements
    # Will be dedicated to 0 or 1 pixel values
    # The goal is to bundle these things
...
def test_hypercorex_char_recog_data_slice(simulator, parameters, waves):
    ...
    module = "test_hypercorex_char_recog_data_slice"
