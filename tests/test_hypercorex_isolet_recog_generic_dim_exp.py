...
# Some parameters about the isoletacter recognition set
NUM_CLASSES = 26
# Note we pruned it from 617 -> 616 to be divisible by 2
NGRAM_GROUP = 2
NUM_FEATURES = 616
NUM_GROUP_FEATURES = NUM_FEATURES // NGRAM_GROUP
NUM_PREDICTIONS = 10
NUM_HV_EXPANSION = 16
...
async def tb_hypercorex_dut(dut):
    cocotb.log.info(" ------------------------------------------ ")
    cocotb.log.info("         Testing ISOLET Recognition         ")
    cocotb.log.info(" ------------------------------------------ ")

    # Extract data set
    assoc_mem_fp = get_dir() + "/../hemaia/trained_am/hypx_isolet_am_multi.txt"
    ...
    # Extract samples
    test_samples_fp = get_dir() + "/../hemaia/test_samples/hypx_isolet_nsample_test.txt"
    cocotb.log.info(f"Get trained AM: {test_samples_fp}")
    test_samples_orig = load_dataset(test_samples_fp)
    # Take out the last feature so it is divisible by 2
    for i in range(len(test_samples_orig)):
        test_samples_orig[i] = test_samples_orig[i][0:NUM_FEATURES]
    test_samples = [row for row in test_samples_orig for _ in range(NUM_HV_EXPANSION)]

    # Compress 8-bit data into a 64-bit data
    test_samples_compressed = []
    for i in range(len(test_samples)):
        compressed_sample = pack_ld_to_hd(test_samples[i], 8, 64)
        test_samples_compressed.append(compressed_sample)

    # Extract asm file
    inst_file_path = get_dir() + "/../sw/asm/test_isolet_recog_generic.asm"
    ...
    dut.am_auto_loop_addr_i.value = NUM_CLASSES * NUM_HV_EXPANSION - 1
    ...
    # Load list to A
    test_sub_samples = test_samples_compressed[: NUM_PREDICTIONS * NUM_HV_EXPANSION]
    print(len(test_sub_samples))
    test_sub_samples_len = len(test_samples_compressed[0])
    for i in range(NUM_PREDICTIONS * NUM_HV_EXPANSION):
        await load_im_list(
    ...
    cocotb.log.info(" ------------------------------------------ ")
    cocotb.log.info("         Write to Number of Classes         ")
    cocotb.log.info(" ------------------------------------------ ")
    ...
    # Write loop control
    inst_loop_mode = 0x0000_0002  # Enable 3D loop
    hv_loop_select = 1 << 3  # HV extend loop sel is 2 but coded 1
    hw_extend_loop = NUM_HV_EXPANSION << 5
    loop_ctrl_code = inst_loop_mode | hv_loop_select | hw_extend_loop
    ...
    loop_count = await config_inst_loop_count(
        ...
        val1=NUM_GROUP_FEATURES,
        val2=NUM_PREDICTIONS * NUM_HV_EXPANSION,
    )
    ...
    # Set port A to be the data slicing mode in 8bit sequence
    # Set port B to be coming from the auto generator
    data_src_ctrl = 0x0000_0023
    ...
    data_src_auto_start_b = 0
    data_src_auto_num_b = NUM_GROUP_FEATURES
    ...
    cocotb.log.info(" ------------------------------------------ ")
    cocotb.log.info("      For now, manually checked output      ")
    cocotb.log.info(" ------------------------------------------ ")

    for i in range(NUM_PREDICTIONS):
        predict_val = await read_predict(dut, i)
        print(f"Prediction {i}: {predict_val}")
    ...
    [
        {
            ...
            "TbMemDepthLowDimA": str(NUM_FEATURES * NUM_HV_EXPANSION * 10),
            "TbAMMemDepth": str(NUM_CLASSES * NUM_HV_EXPANSION),
        }
    ],
)
def test_hypercorex_isolet_recog_generic_dim_exp(simulator, parameters, waves):
    ...
    module = "test_hypercorex_isolet_recog_generic_dim_exp"
